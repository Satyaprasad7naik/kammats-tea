  ...
  if (event.type === 'checkout.session.completed') {
    ...
    try {
      await prisma.$transaction(async (tx) => {
        ...
        const existingOrders: any[] = await tx.$queryRaw`SELECT "paymentStatus" FROM "Order" WHERE "stripeSessionId" = ${sessionId} FOR UPDATE`;
     ...
     try {
       await prisma.$transaction(async (tx) => {
         const existingOrders: any[] = await tx.$queryRaw`SELECT id, "paymentStatus" FROM "Order" WHERE "stripeSessionId" = ${sessionId} FOR UPDATE`;
