...
const AdminDashboard = () => {
  ...
  const fetchOrders = async (token: string) => {
    setLoading(true);
    try {
      // In a real app we would have an admin protected route for this
      // For now we'll fetch all orders directly if we have a token
      // I'll quickly add a generic order fetch to the backend later, or just mock it here if not available
    ...
    <div className="min-h-screen bg-gray-50 text-gray-900 font-sans">
      ...
      <div className="max-w-7xl mx-auto p-6">
        <div className="flex justify-between items-center mb-6">
          ...
          <button
            onClick={() => fetchOrders(localStorage.getItem('admin-token') || '')}
